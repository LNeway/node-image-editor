...
(async () => {
  ...
  for (const color of COLORS) {
    // Try to find color inputs
    const inputs = page.locator('input[type="number"]');
    const count = await inputs.count();
    
    if (count >= 3) {
      await inputs.nth(0).fill(color.r.toString());
      await inputs.nth(1).fill(color.g.toString());
      await inputs.nth(2).fill(color.b.toString());
    } else {
      // Try hex input
      const hex = '#' + color.r.toString(16).padStart(2,'0') + color.g.toString(16).padStart(2,'0') + color.b.toString(16).padStart(2,'0');
      await page.locator('input[type="text"]').first().fill(hex);
    }
