...
(async () => {
  ...
  // Click on the added node on canvas to select it and show properties panel
  ...
  // Click the node on canvas (last one)
  if (count > 1) {
  ...
  for (const color of COLORS) {
    try {
      // Find number inputs for RGB (there should be 3)
      const numberInputs = page.locator('input[type="number"]');
      const numCount = await numberInputs.count();
      console.log(`Testing ${color.name}: ${numCount} number inputs found`);
      
      if (numCount >= 3) {
        // Fill R, G, B values
        await numberInputs.nth(0).fill(color.r.toString());
        await numberInputs.nth(1).fill(color.g.toString());
        await numberInputs.nth(2).fill(color.b.toString());
        
        // Trigger change event
        await numberInputs.nth(0).dispatchEvent('change');
        await page.waitForTimeout(1000);
      } else {
        console.log(`Not enough inputs for ${color.name}`);
      }
