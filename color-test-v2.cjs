...
async function sampleCenterPixel(page) {
 ...
 isColorMatch(pixel, expected, tolerance = 10) {
  return Math.abs(pixel  });
}

function.r - expected.r) <= tolerance &&
...
(async () => {
  ...
  for (const color of COLORS) {
    try {
      // Try to use textbox with hex color
      const hex = '#' + color.r.toString(16).padStart(2,'0') + color.g.toString(16).padStart(2,'0') + color.b.toString(16).padStart(2,'0');
      
      // Find the text input (hex color)
      const textInputs = page.locator('input[type="text"]');
      const textCount = await textInputs.count();
      console.log(`Color ${color.name}: ${textCount} text inputs found`);
      ...
      results.push({ ...color, status: 'FAIL', reason: e.message, pixel: {} });
    }
  }
  ...
  for (const r of results) {
    ...
    console.log(`[${r.status}] ${status} ${r.name}: Expected RGB(${r.r},${r.g},${r.b}), Got RGB(${r.pixel?.r},${r.pixel?.g},${r.pixel?.b})`);
